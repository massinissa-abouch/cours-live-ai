...
export const Route = createFileRoute("/_authenticated/tools/homework")({
  ...
  errorComponent: HomeworkErrorComponent,
  pendingComponent: () => (
...
function HomeworkErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  console.error("[homework]", error);
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center gap-4 px-4 text-center">
      <h1 className="text-lg font-semibold">Une erreur est survenue</h1>
      <p className="text-sm text-muted-foreground">
        {error?.message ?? "Impossible d'afficher le cahier de textes."}
      </p>
